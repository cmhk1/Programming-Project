...
plt.scatter(x=sharpe_portfolio['Volatility'], y=sharpe_portfolio['Returns'], c='red', marker='D', s=200, label = 'Max-sharpe-ratio portfolio')
    # plot green mark to highlight position of portfolio with the lowest variance
plt.scatter(x=min_variance_port['Volatility'], y=min_variance_port['Returns'], c='blue', marker='D', s=200, label = 'Min-variance portfolio')
plt.legend()
plt.show()

# print the details of the two desired portfolios
print("The portoflio with the maximum sharpe ratio is:\n", sharpe_portfolio.T)
print("The portfolio with the minimum variance is:\n", min_variance_port.T)
